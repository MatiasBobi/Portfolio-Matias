...
const KnowledgeCard = ({ type, stack }: KnowledgeCardProps) => {
  return (
    <div className="bg-[#131212] border border-gray-700 rounded-3xl p-6 hover:border-yellow-400/70 hover:-translate-y-1 hover:shadow-[0_0_30px_-10px_rgba(253,213,0,0.25)] hover:transition-all duration-300">
      <h3 className="text-2xl font-semibold text-[#FDD500] mb-6 tracking-wide">{type}</h3>
      ...
      <div className="flex flex-wrap gap-4 justify-center items-center">
        {stack.map((technology) => {
          ...
          return (
            <div
              key={technology.name}
              className=" flex items-center gap-2 bg-gray-900/80 px-4 py-2 rounded-xl transition-all duration-200 hover:bg-gray-800"
            >
              {IconName && (
                <IconName className="text-2xl" style={{ color: iconColor }} />
              )}
              <span className="text-white text-lg tracking-wide">{technology.name}</span>
            </div>
          );
        })}
      </div>
